...
public:
    feature(size_t len)
        : length(len), weight(alloc(len))
    {
    }

    feature(feature&& f)
        : length(f.length), weight(f.weight)
    {
        f.weight = nullptr;
    }
...
public:
    pattern(const std::vector<int>& p, int iso = 8)
        : feature(1 << (p.size() * 4)), iso_last(iso)
    {
        if (p.empty()) {
...
public:
    board after_state() const
    {
        return after;
    }

    board before_state() const
    {
        return before;
    }
    ...
    void set_before_state(const board& b)
    {
        before = b;
    }

    void set_after_state(const board& b)
    {
        after = b;
    }
